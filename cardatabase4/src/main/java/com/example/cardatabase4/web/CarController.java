...
public class CarController {
    ...
    // 3. 차량 한 대 조회
    @GetMapping("/cars/{id}")
    public ResponseEntity<Car> getCarById(@PathVariable Long id){ // id가 숫자라면 자료형을 Long으로 바꿔준다.
        return carService.getCarById(id)
                ...
                .orElse(ResponseEntity.notFound().build());     // 없으면 404 not Found를 return 시킬 것 ,
                                                                // 이 전체 return문은 ResponseEntity 자료형을 가지게 됨.
    }
    ...
    public ResponseEntity<Void> deleteCar(@PathVariable Long id) {
        if (carService.deleteCar(id)){
            return ResponseEntity.noContent().build(); // 삭제 성공시 204 Content return
        } else {
